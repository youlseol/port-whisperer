...
pub fn detect(project_root: &Path) -> Option<String> {
    ...
    if let Ok(content) = std::fs::read_to_string(&pkg_path) {
        if let Ok(pkg) = serde_json::from_str::<Value>(&content) {
            ...
            if all_keys.contains(&"next") {
                return Some("Next.js".into());
            }
            if all_keys.contains(&"nuxt") {
                return Some("Nuxt".into());
            }
            if all_keys.contains(&"@sveltejs/kit") {
                return Some("SvelteKit".into());
            }
            if all_keys.contains(&"vite") {
                return Some("Vite".into());
            }
            if all_keys.contains(&"react-scripts") {
                return Some("Create React App".into());
            }
            if all_keys.contains(&"@angular/core") {
                return Some("Angular".into());
            }
            if all_keys.contains(&"express") {
                return Some("Express".into());
            }
            if all_keys.contains(&"fastify") {
                return Some("Fastify".into());
            }
            if all_keys.contains(&"@nestjs/core") {
                return Some("NestJS".into());
            }
            if all_keys.contains(&"gatsby") {
                return Some("Gatsby".into());
            }
            if all_keys.contains(&"remix") {
                return Some("Remix".into());
            }
            if all_keys.contains(&"astro") {
                return Some("Astro".into());
            }
            if all_keys.contains(&"@remix-run/node") {
                return Some("Remix".into());
            }
            if all_keys.contains(&"hapi") || all_keys.contains(&"@hapi/hapi") {
                return Some("Hapi".into());
            }
    ...
    if project_root.join("mix.exs").exists() {
        return Some("Phoenix".into());
    }
    if project_root.join("Gemfile").exists() {
        return Some("Ruby/Rails".into());
    }
